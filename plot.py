...
def get_points(fp : str, centerpoint) -> tuple[tuple[float, float], list[float], list[float]]:
    xvals = []
    yvals = []
    with open(fp) as f:
    ...
    return (centerpoint, xvals, yvals)
...
if __name__ == '__main__':
    ...
    if (os.path.isfile(fp)):
        centerpoint, xp, yp = get_points(fp, None)
        filedat.append([xp, yp]) 
        ...
        for f in logs:
            ...
            centerpoint, xp, yp = get_points(join(fp, f), centerpoint)
            if (centerpoint == None or len(xp) == 0):
                print("Error reading file " + str(f))
            else: 
                filedat.append([f, xp, yp]) 
    else:
